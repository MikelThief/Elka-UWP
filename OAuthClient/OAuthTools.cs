...
namespace OAuthClient
{
    ...
    public static class OAuthTools
    {
        ...
        static OAuthTools()
        {
            ...
            _rng.GetBytes(data: bytes);
            _random = new Random(Seed: BitConverter.ToInt32(value: bytes, startIndex: 0));
#else
            _random = new Random();
#endif
        }
        ...
        public static string GetNonce()
        {
            ...
            lock (_randomLock)
            {
                ...
                {
                    nonce[i] = chars[index: _random.Next(0, maxValue: chars.Length)];
                }
            }
            return new string(value: nonce);
        }
        ...
        {
            return GetTimestamp(dateTime: DateTime.UtcNow);
        }
        ...
        {
            var timestamp = ToUnixTime(dateTime: dateTime);
        ...
        {
            var escaped = Uri.EscapeDataString(stringToEscape: value);

            // LinkedIn users have problems because it requires escaping brackets
            escaped = escaped.Replace("(", newValue: PercentEncode("("))
                             .Replace(")", newValue: PercentEncode(")"));
        ...
        {
            var bytes = Encoding.UTF8.GetBytes(s: s);
            ...
            foreach (var b in bytes)
            {
                ...
                if ((b > 7 && b < 11) || b == 13)
                {
                    sb.Append(value: string.Format("%0{0:X}", arg0: b));
                }
                else
                {
                    sb.Append(value: string.Format("%{0:X}", arg0: b));
                }
            }
            return sb.ToString();
        }
        ...
        public static string UrlEncodeStrict(string value)
        {
            ...
            var ret = original.OfType<char>().Where(
                c => !Unreserved.OfType<char>().Contains(value: c) && c != '%').Aggregate(
                    seed: value, func: (current, c) => current.Replace(
                          oldValue: c.ToString(), newValue: PercentEncode(s: c.ToString())
                          ));
        ...
        {
            var copy = SortParametersExcludingSignature(parameters: parameters);
            var concatenated = Concatenate(collection: copy, separator: "=", spacer: "&");
            return concatenated;
        }
        ...
        {
            var copy = new WebParameterCollection(parameters: parameters);
            var exclusions = copy.Where(n => EqualsIgnoreCase(left: n.Name, right: "oauth_signature"));

            copy.RemoveAll(parameters: exclusions);
        ...
        public static string ConstructRequestUrl(Uri url)
        {
            ...
            var requestUrl = string.Format("{0}://{1}", arg0: url.Scheme, arg1: url.Host);
            var qualified = string.Format(":{0}", arg0: url.Port);
            var basic = url.Scheme == "http" && url.Port == 80;
            var secure = url.Scheme == "https" && url.Port == 443;

            sb.Append(value: requestUrl);
            sb.Append(value: !basic && !secure ? qualified : "");
            sb.Append(value: url.AbsolutePath);
        ...
        public static string ConcatenateRequestElements(string method, string url, WebParameterCollection parameters)
        {
            ...
            sb.Append(value: requestUrl);
            sb.Append(value: requestParameters);
        ...
        {
            return GetSignature(signatureMethod: signatureMethod, signatureTreatment: OAuthSignatureTreatment.Escaped, signatureBase: signatureBase, consumerSecret: consumerSecret, tokenSecret: null);
        }
        ...
        public static string GetSignature(OAuthSignatureMethod signatureMethod,
                                          ...
                                          string consumerSecret)
        {
            return GetSignature(signatureMethod: signatureMethod, signatureTreatment: signatureTreatment, signatureBase: signatureBase, consumerSecret: consumerSecret, tokenSecret: null);
        }
        ...
        public static string GetSignature(OAuthSignatureMethod signatureMethod,
                                          ...
                                          string tokenSecret)
        {
            return GetSignature(signatureMethod: signatureMethod, signatureTreatment: OAuthSignatureTreatment.Escaped, signatureBase: consumerSecret, consumerSecret: tokenSecret);
        }
                                          ...
                                          string tokenSecret)
        {
            ...
            switch (signatureMethod)
            {
                case OAuthSignatureMethod.HmacSha1:
                    {
                        consumerSecret = UrlEncodeRelaxed(value: consumerSecret);
                        tokenSecret = UrlEncodeRelaxed(value: tokenSecret);
                        var key = string.Concat(str0: consumerSecret, str1: "&", str2: tokenSecret);
#if WINRT
            ...
            var result = signatureTreatment == OAuthSignatureTreatment.Escaped
                       ? UrlEncodeRelaxed(value: signature)
                       : signature;
        ...
        {
            var data = Encoding.UTF8.GetBytes(s: input);
            var hash = algorithm.ComputeHash(buffer: data);
